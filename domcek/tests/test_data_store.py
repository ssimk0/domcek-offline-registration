...
def test_data_store():
    ...
    with open( path.join(getcwd(), 'domcek/test_data/sample_data.json'), 'w+') as outfile:
        json.dump(data, outfile, indent=4)
    class testData(DataStore):
        data_path = path.join(getcwd(), 'domcek/test_data/sample_data.json')
...
def empty_data_store():
    data_path = path.join(getcwd(), 'domcek/test_data/wrong_path_data.json')
    ## cleanup before test
...
def test_filter_data(test_data_store):
    ...
    data = test_data_store.filter('John Smith')
